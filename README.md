...
| Экран P4 | LVGL 9.5, 1024×600: CPU Seed3, IN/OUT levels, USB status |
...
![Главный экран аудиоинтерфейса: загрузка Seed3, отдельные частоты и четыре индикатора уровня](ui/seed3_p4_ui/preview.svg)
...
- последнюю частоту, с которой Windows действительно открывала каждый endpoint;
- состояние USB HS, размер буфера и накопительные ошибки транспорта.

Частоты на экране запоминаются раздельно. Например, после открытия входа на
96 кГц и выхода на 44,1 кГц журнал и экран показывают `96 / 44.1`, а не дважды
последнее значение. При этом физический аудиотакт Seed3 остаётся общим:
одновременный full-duplex без ASRC требует одинаковой частоты обоих потоков.
