...
enum { CAP = 0, PLAY = 1, FB = 2, SLOT_COUNT = 4, DMA_STRIDE = 448,
       FREE = 0, READY, IN_FLIGHT, DONE, BUILDING };
typedef struct { uint16_t bytes; uint8_t state; } slot_t;
typedef struct {
    uint8_t address;
    int8_t current;
...
static bool suspended, capture_started;
...
static unsigned interval(void) { return tud_speed_get() == TUSB_SPEED_HIGH ? 8 : 1; }
...
static void arm(unsigned index, uint16_t now) {
    ...
    if (index == CAP) {
        ...
        if (e->slots[slot].state == READY) {
            ...
            if (!capture_started) return; // first packet waits for 96 real frames
            e->current = -1;
            buffer = silence_dma;
            bytes = 48 * 8; // active capture NEVER queues a ZLP
        }
    } else if (index == PLAY) {
...
static bool transfer_isr(uint8_t port, uint8_t address, xfer_result_t result, uint32_t bytes) {
    ...
    if (result == XFER_RESULT_SUCCESS) {
        ...
        if (index == CAP) {
            ...
            ADD(stats.capture_completed_frames, bytes / 8);
            if (bytes == 47 * 8) ADD(stats.capture_short_packets, 1);
            if (bytes == 49 * 8) ADD(stats.capture_long_packets, 1);
            if (e->current >= 0) {
                slot_t *s = &e->slots[e->current];
                queued_frames -= s->bytes / 8;
                s->state = FREE;
            } else ADD(stats.capture_silence_frames, bytes / 8);
        } else if (index == PLAY) {
            ADD(stats.playback_packets, 1);
            if (bytes < 384) ADD(stats.playback_short_packets, 1);
            if (bytes > 384) ADD(stats.playback_long_packets, 1);
            if (e->current >= 0) {
                slot_t *s = &e->slots[e->current];
                s->bytes = bytes;
                s->state = DONE;
                ++e->produced;
            } else ADD(stats.playback_overrun_frames, bytes / 8);
            ADD(stats.playback_received_frames, bytes / 8);
        } else ADD(stats.feedback_packets, 1);
        // Completion learns host phase. Arm one uframe before the next poll,
        // not in every intervening uframe (bInterval=4 => eight HS uframes).
...
static bool endpoint_restart(unsigned index, bool enable) {
    ...
    bool ok = usbd_edpt_iso_activate(UAC_PORT, uac_endpoint(eps[index].address));
    if (ok) {
        state_clear(index);
...
static bool stream_set(unsigned itf, uint8_t alt) {
    ...
    if (itf == UAC_CAP_ITF) {
        STORE(cap_active, 0);
        if (!endpoint_restart(CAP, enable)) return false;
        queued_frames = 0;
        ...
        STORE(play_active, 0);
        if (!endpoint_restart(PLAY, enable) || !endpoint_restart(FB, enable)) return false;
        ADD(play_epoch, 1); // playback consumer owns and flushes its tail
        fill_q8 = P4_UAC2_PREFILL << 8;
...
static uint16_t driver_open(uint8_t port, const tusb_desc_interface_t *itf, uint16_t length) {
    ...
    rate_q16 = tud_speed_get() == TUSB_SPEED_HIGH ? 6u << 16 : 48u << 16;
...
static bool control_impl(uint8_t port, uint8_t stage, const tusb_control_request_t *r) {
    ...
    if (r->bmRequestType_bit.type == TUSB_REQ_TYPE_STANDARD) {
        ...
        if (r->bRequest == TUSB_REQ_SET_INTERFACE && !input && r->wValue <= 1 && !r->wLength) {
    ...
    if (entity == 4 && channel == 0 && input) {
        if (selector == AUDIO_CS_CTRL_SAM_FREQ) {
            if (r->bRequest == AUDIO_CS_REQ_CUR && r->wLength == 4) {
                const uint32_t rate = P4_UAC2_RATE;
                memcpy(control_buffer, &rate, 4);
                return tud_control_xfer(port, r, control_buffer, 4);
            }
            if (r->bRequest == AUDIO_CS_REQ_RANGE) {
                const uint8_t range[] = {1, 0, 0x80, 0xbb, 0, 0, 0x80, 0xbb, 0, 0, 0, 0, 0, 0};
                memcpy(control_buffer, range, sizeof(range));
                return tud_control_xfer(port, r, control_buffer, sizeof(range));
            }
        }
        if (selector == AUDIO_CS_CTRL_CLK_VALID && r->bRequest == AUDIO_CS_REQ_CUR && r->wLength == 1) {
            control_buffer[0] = last_source && (uint32_t)(esp_timer_get_time() / 1000 - last_source_ms) < 100;
...
static void capture_prepare(void) {
    ...
    if (!capture_started) {
        if (fill < P4_UAC2_PREFILL) return;
    ...
    for (unsigned count = 0; count < 2; ++count) {
        ...
        // Retain ~96 frames ahead of the host plus one 48-frame DMA packet.
        // The source arrives in 32-frame bursts, so use a full packet of headroom.
        unsigned frames = audio_packet_frames(occupancy > 48 ? occupancy - 48 : 0, &packet_mode);
        if (audio_ring_fill(&capture_ring) < frames) { usbd_spin_unlock(false); break; }
        e->slots[slot].state = BUILDING;
        usbd_spin_unlock(false);
        audio_ring_read(&capture_ring, capture_dma[slot], frames); // task-only PCM memcpy
        usbd_spin_lock(false);
        e->slots[slot].bytes = frames * 8;
...
static void playback_drain(void) {
    ...
    for (unsigned count = 0; count < SLOT_COUNT; ++count) {
        ...
        usbd_spin_unlock(false);
        if (!ready) break;
        if (bytes > UAC_MAX_BYTES || bytes % 8) ADD(stats.playback_bad_packets, 1);
        else if (LOAD(play_active)) {
            ...
            for (unsigned f = 0; f < frames; ++f) {
                for (unsigned c = 0; c < 2; ++c) {
                    pcm[f * 2 + c] = (LOAD(muted[0]) || LOAD(muted[c + 1])) ? 0 :
                        (int32_t)((uint32_t)playback_dma[slot][f * 2 + c] & 0xffffff00u);
                    if (gains[c] != (1 << 24))
...
static void feedback_update(uint32_t now_ms) {
    ...
    int32_t error = P4_UAC2_PREFILL - (int32_t)(fill_q8 >> 8);
    ...
    // u_audio_set_fback_frequency(). At HS bInterval=4 nominal wire value is 48.
...
static void watchdog(void) {
    ...
    for (unsigned i = 0; i < 3; ++i) {
        ...
        if (stalled) {
            ...
            if (i == CAP) stream_set(UAC_CAP_ITF, 1);
            else if (i == PLAY) stream_set(UAC_PLAY_ITF, 1);
...
static void usb_task(void *arg) {
    (void)arg;
    if (!audio_ring_self_test(&capture_ring)) {
        STORE(initialised, 3); ESP_LOGE("p4_uac2", "SPSC self-test FAILED"); vTaskDelete(NULL); return;
    }
    ESP_LOGI("p4_uac2", "SPSC wrap/overflow/order and 47/48/49 controller self-tests PASS");
...
size_t p4_uac2_playback_read(int32_t *pcm, size_t frames) {
    ...
    if (epoch != current_epoch || !LOAD(play_active)) {
    ...
    if (!started) {
        if (fill < P4_UAC2_PREFILL) return 0;
        started = true;
    }
    if (fill < frames) {
