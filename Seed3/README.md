...
- `BALANCED`: 32 стереокадра до 48 кГц и 64 на 88,2/96 кГц.
- `LOW LATENCY`: 32 кадра на всех частотах.
- `LOCAL`: АЦП/ЦАП и локальный граф работают, но аудио по SPI не копируется;
  примерно раз в 20 мс передаются только управление и телеметрия.
...
В автоматическом `LOCAL` при 48 кГц измерено около 3% CPU. На 96 кГц
`BALANCED` занимает около 23%, `LOW LATENCY` — около 26%.

[Подробно о режимах и протоколе](../docs/TRANSPORT_MODES_RU.md).
