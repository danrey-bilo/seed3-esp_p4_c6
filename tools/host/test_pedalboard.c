...
static void check_layout(const SeedFxGraphDefinition *g)
{
    pedal_layout_t layout;
    pedal_layout_build(&layout,g,3,3);
...
static void test_layout(void)
{
    ...
    puts("PASS: 1536 stereo/mixed/wide layouts, reversed IDs, no overlapping parallel segments, canvas bounds");
}
...
int main(void)
{
    ...
    assert(pedal_gesture_update(&gesture,1099,50,180)==PEDAL_GESTURE_NONE);
    assert(pedal_gesture_update(&gesture,1100,105,180)==PEDAL_GESTURE_DRAG);
    assert(pedal_gesture_update(&gesture,2200,20,0)==PEDAL_GESTURE_DRAG);
    pedal_gesture_start(&gesture,0,0,0,104,210);
    assert(pedal_gesture_update(&gesture,1999,90,180)==PEDAL_GESTURE_NONE);
    assert(pedal_gesture_update(&gesture,2000,2,0)==PEDAL_GESTURE_MENU);
    pedal_gesture_start(&gesture,0,0,0,104,210);
    ...
    uint16_t first=s_visual_order[0];s_visual_order[0]=s_visual_order[1];s_visual_order[1]=first;
    ...
    assert(lv_obj_get_width(s_node_cards[cabinet])==208);
    assert(lv_obj_get_width(s_node_cards[0])==104);
    ...
    test_pressed=true;touch_advance(touch,700);
    test_touch.x+=20;test_touch.y+=14;touch_advance(touch,1100);
    ...
    test_pressed=true;touch_advance(touch,1100);
    ...
    unsigned previous_slot=s_visual_slot[0];ui_graph(&before);
    test_pressed=false;touch_advance(touch,30);ui_graph(&after);
    assert(s_visual_slot[0]!=previous_slot && !memcmp(&before,&after,sizeof(before)));
    assert(lv_obj_has_flag(s_node_context,LV_OBJ_FLAG_HIDDEN));lv_indev_delete(touch);
    ...
    puts("PASS: categories/Other, LED toggle, wide cabinet, drag timing, visual-only reorder, Wi-Fi connected view and password clearing");
    return 0;
}
