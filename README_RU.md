...
Каталог показывает ориентировочную нагрузку Seed3. Педаль получает свой цвет и
форму. Большая лампа сверху включает/отключает эффект: зелёная — active,
красная — bypass. Отдельной кнопки и номера педали нет. Удержание 2 секунды
внутри корпуса открывает `EDIT`, `REPLACE`, `INFORMATION` и `DELETE`.
Удержание 1 секунду с выходом пальца за корпус меняет место карточки в сетке,
...
разнесены; для плотной схемы доступна вертикальная прокрутка. Уровни слева
и справа показывают физические АЦП/ЦАП Seed3, даже без USB-потока.

Кабинет занимает двойную ширину. Текущий Cabinet Sim имеет три действующие
