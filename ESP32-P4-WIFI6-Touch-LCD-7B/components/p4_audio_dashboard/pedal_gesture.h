...
enum { PEDAL_DRAG_HOLD_MS=500, PEDAL_MENU_HOLD_MS=1000, PEDAL_TAP_MAX_MS=500 };
...
static inline unsigned pedal_gesture_update(pedal_gesture_t *g,uint32_t now,int x,int y)
{
    ...
    if(outside && elapsed<PEDAL_DRAG_HOLD_MS) { g->cancelled=true; return PEDAL_GESTURE_NONE; }
    if(g->dragging || (outside && elapsed>=PEDAL_DRAG_HOLD_MS)) {
        g->dragging=true; return PEDAL_GESTURE_DRAG;
    }
    if(elapsed>=PEDAL_MENU_HOLD_MS) { g->menu=true; return PEDAL_GESTURE_MENU; }
