...
typedef struct {
    ...
    bool spk_active;
    bool mic_active;
    volatile uint64_t mic_tx_callbacks;
    volatile uint64_t mic_tx_bytes;
    volatile uint64_t mic_fifo_write_bytes;
    volatile uint64_t mic_fifo_short_writes;
} uac_device_t;
...
bool tud_audio_set_itf_cb(uint8_t rhport, tusb_control_request_t const *p_request)
{
    ...
    if (s_uac_device->mic_itf_num == itf && alt != 0) {
        ...
        // The working Seed3 USB implementation does not arm its first ISO IN
        // packet until the capture ring has reached a target fill.  Apply the
        // same rule to TinyUSB's class FIFO: start near its midpoint so flow
        // control cannot emit a zero-length packet while the producer task is
        // waking up.
        const size_t bytes_require =
            MIC_INTERVAL_MS * s_uac_device->mic_bytes_per_ms;
        tu_fifo_t *fifo = tud_audio_n_get_ep_in_ff(0);
        const size_t target_fill =
            CFG_TUD_AUDIO_FUNC_1_EP_IN_SW_BUF_SZ / 2U;
        while (fifo != NULL && tu_fifo_count(fifo) < target_fill &&
               s_uac_device->user_cfg.input_cb != NULL) {
            size_t bytes_read = 0;
            const esp_err_t ret = s_uac_device->user_cfg.input_cb(
                s_uac_device->mic_buf_write, bytes_require, &bytes_read,
                s_uac_device->user_cfg.cb_ctx);
            if (ret != ESP_OK || bytes_read == 0U) {
                break;
            }
            const uint16_t written = tud_audio_n_write(
                0, s_uac_device->mic_buf_write, bytes_read);
            __atomic_fetch_add(&s_uac_device->mic_fifo_write_bytes, written,
                               __ATOMIC_RELAXED);
            if (written != bytes_read) {
                __atomic_fetch_add(&s_uac_device->mic_fifo_short_writes, 1U,
                                   __ATOMIC_RELAXED);
                break;
            }
        }
...
bool tud_audio_rx_done_isr(uint8_t rhport, uint16_t n_bytes_received, uint8_t func_id, uint8_t ep_out, uint8_t cur_alt_setting)
{
    ...
    // The application callback only copies into a lock-free PCM ring. Run it
    // directly in the TinyUSB task instead of adding a second task wake-up and
    // up to one scheduler tick of playback latency.
    if (s_uac_device->spk_data_size > 0 && s_uac_device->user_cfg.output_cb) {
        s_uac_device->user_cfg.output_cb(
            (uint8_t *)s_uac_device->spk_buf,
            s_uac_device->spk_data_size,
            s_uac_device->user_cfg.cb_ctx);
    }
    s_uac_device->spk_data_size = 0;
    return true;
...
bool tud_audio_tx_done_isr(uint8_t rhport, uint16_t n_bytes_sent, uint8_t func_id, uint8_t ep_in, uint8_t cur_alt_setting)
{
    ...
    (void)ep_in;
    (void)cur_alt_setting;
    // Keep this callback constant-time and stack-free.  The producer task
    // fills TinyUSB's SPSC FIFO directly; the DCD drains it here.
    (void)func_id;
    __atomic_fetch_add(&s_uac_device->mic_tx_callbacks, 1U,
                       __ATOMIC_RELAXED);
    __atomic_fetch_add(&s_uac_device->mic_tx_bytes, n_bytes_sent,
                       __ATOMIC_RELAXED);
    return true;
}
...
static void usb_mic_task(void *pvParam)
{
    ...
    TickType_t last_log_time = last_wake_time;
    while (1) {
        ...
        if (s_uac_device->user_cfg.input_cb) {
            ...
            if (ret != ESP_OK) {
                ...
                /*
                 * Match TinyUSB's proven audio_test_freertos producer: push
                 * one complete millisecond into the class FIFO every tick and
                 * let the audio class perform HS packet sizing/flow control.
                 *
                 * Do not gate the write with tu_fifo_remaining().  The audio
                 * FIFO is intentionally configured as overwritable; a
                 * producer-side check races the USB consumer and can leave the
                 * stream permanently starved after the first FIFO wrap.
                 */
                const uint16_t written = tud_audio_n_write(
                    0, s_uac_device->mic_buf_write, bytes_read);
                __atomic_fetch_add(&s_uac_device->mic_fifo_write_bytes,
                                   written, __ATOMIC_RELAXED);
                if (written != bytes_read) {
                    __atomic_fetch_add(
                        &s_uac_device->mic_fifo_short_writes, 1U,
                        __ATOMIC_RELAXED);
                }
            }
        }
        vTaskDelayUntil(&last_wake_time, pdMS_TO_TICKS(MIC_INTERVAL_MS));

        if (xTaskGetTickCount() - last_log_time >= pdMS_TO_TICKS(1000)) {
            last_log_time = xTaskGetTickCount();
            tu_fifo_t *fifo = tud_audio_n_get_ep_in_ff(0);
            ESP_LOGI(TAG,
                     "mic tx_cb=%" PRIu64 " tx_bytes=%" PRIu64
                     " write_bytes=%" PRIu64 " short=%" PRIu64
                     " fifo=%u/%u",
                     __atomic_load_n(&s_uac_device->mic_tx_callbacks,
                                     __ATOMIC_RELAXED),
                     __atomic_load_n(&s_uac_device->mic_tx_bytes,
                                     __ATOMIC_RELAXED),
                     __atomic_load_n(&s_uac_device->mic_fifo_write_bytes,
                                     __ATOMIC_RELAXED),
                     __atomic_load_n(&s_uac_device->mic_fifo_short_writes,
                                     __ATOMIC_RELAXED),
                     fifo == NULL ? 0U : (unsigned)tu_fifo_count(fifo),
                     fifo == NULL ? 0U : (unsigned)tu_fifo_depth(fifo));
        }
    }
}
#endif
...
#if MIC_CHANNEL_NUM
    ret_val = xTaskCreatePinnedToCore(usb_mic_task, "usb_mic_task", 4096,
                                      NULL, CONFIG_UAC_MIC_TASK_PRIORITY,
                                      &s_uac_device->mic_task_handle,
                                      CONFIG_UAC_MIC_TASK_CORE == -1
                                          ? tskNO_AFFINITY
                                          : CONFIG_UAC_MIC_TASK_CORE);
    ESP_RETURN_ON_FALSE(ret_val == pdPASS, ESP_FAIL, TAG,
                        "Failed to create usb_mic task");
#endif
