...
#define PLAY_ALT(alt, subslot, bits, interval) \
    ...
    TUD_AUDIO_DESC_TYPE_I_FORMAT(subslot, bits), \
    TUD_AUDIO_DESC_STD_AS_ISO_EP(UAC_PLAY_EP, 5, UAC_MAX_FRAMES * 2 * subslot, interval), \
    TUD_AUDIO_DESC_CS_AS_ISO_EP(0, 0, 0, 0), \
    TUD_AUDIO_DESC_STD_AS_ISO_FB_EP(UAC_FB_EP, 4, interval)
#define CAP_ALT(alt, subslot, bits, interval) \
    TUD_AUDIO_DESC_STD_AS_INT(2, alt, 1, 6), \
    TUD_AUDIO_DESC_CS_AS_INT(0x13, 0, AUDIO_FORMAT_TYPE_I, AUDIO_DATA_FORMAT_TYPE_I_PCM, 2, 3, 0), \
    TUD_AUDIO_DESC_TYPE_I_FORMAT(subslot, bits), \
    TUD_AUDIO_DESC_STD_AS_ISO_EP(UAC_CAP_EP, 5, UAC_MAX_FRAMES * 2 * subslot, interval), \
    TUD_AUDIO_DESC_CS_AS_ISO_EP(0, 0, 0, 0)
#define PROFILE(interval) \
    ...
    /* External programmable Seed clock: frequency RW, validity R. */ \
    TUD_AUDIO_DESC_CLK_SRC(4, 0, 7, 0, 0), \
...
const tusb_desc_endpoint_t *uac_endpoint(uint8_t ep, uint8_t alternate) {
    ...
    for (size_t n = 0; n < sizeof(hs); n += p[n]) {
        if (p[n + 1] == TUSB_DESC_INTERFACE) current_alternate = p[n + 3];
        if (p[n + 1] == TUSB_DESC_ENDPOINT && p[n + 2] == ep && current_alternate == alternate)
