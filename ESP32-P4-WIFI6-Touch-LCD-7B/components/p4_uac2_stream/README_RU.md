...
`p4_uac2_get_stats()` дополнительно возвращает
`capture_sample_rate` и `playback_sample_rate`. Это последние частоты, с
которыми Windows реально открывала соответствующие endpoints; они не
перезаписывают друг друга. Поле `sample_rate` по-прежнему означает текущую
общую физическую частоту Seed3. Такое разделение исправляет диагностику и
интерфейс, не добавляя скрытого ресемплинга в аудиотракт.

`host_alive` в статистике основан на продвижении HS SOF. Это важно для
self-powered платы с разорванным VBUS: после физического снятия D+/D− TinyUSB
может ещё показывать `connected/mounted/suspended`, но отсутствие SOF переводит
систему в LOCAL и запускает однократную переинициализацию контроллера для
replug. Флаг `suspended` намеренно не считается доказательством присутствия ПК:
без линии VBUS USB suspend и физическое снятие D+/D− неразличимы. При возврате
SOF владелец автоматически снова переключается в WINDOWS.
